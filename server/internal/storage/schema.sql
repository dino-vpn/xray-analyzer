...
-- Records operator actions taken from the Attacks tab: Remnawave disable/delete
-- and CrowdSec IP ban/unban. Drives the "Журнал действий" panel and lets the
-- UI show active bans (latest ip_ban per IP without a newer ip_unban).
-- =============================================================================
CREATE TABLE IF NOT EXISTS enforcement_actions (
    id           bigint PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    action_type  text        NOT NULL,   -- remna_disable|remna_delete|ip_ban|ip_unban
    user_email   uuid,
    target_ip    inet,
    duration     text,                   -- '30m','1h',... (ip_ban only)
    reason       text,
    status       text        NOT NULL,   -- success|failed
    error        text,
    decision_ref text,                   -- CrowdSec alert/decision id, when known
    created_at   timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_enf_actions_user ON enforcement_actions(user_email, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_enf_actions_ip   ON enforcement_actions(target_ip, created_at DESC);
